...
describe('Visits the landing page and checks the logo', () => {
  ...
  it('Review table should have 3 rows', () => {
    cy.get('.padded').find('tr').should('have.length', 3)
...
it('Menu should have six items', () => {
  cy.get('.three').children().should('have.length', 6)

})

it('click on two buttons and check the total', () => {
  cy.get('[indexid="0"] > .extra > .two > .green').click()
  cy.get('[indexid="1"] > .extra > .two > .green').click()
  cy.get('th > .ui > :nth-child(2)').invoke('text').should('eq', '$31.90')
  // .expect().to.contain('$31.90')
})

// it('click on payment and get total', () => {
//   cy.get('span').click()
//   cy.wait(1000)
//   // cy.get('Fieldset-input Textbox-control')
...
//     // const myInput = iframe.find("");
//     // cy.wrap(myInput).type("adrian@gmail.com");
...
//   cy.get('.input').type('Job Title')
//   cy.get('[name="pay"]').type('Pay')
